Page({
  ...
  onLoad(options) {
    ...
    this.loadUsers();
    this.logOperation(`查看了${options.region}地区的正在帮教的孩子列表`);
  },
  ...
  async loadUsers() {
    ...
    try {
      ...
      console.log('格式化后的数据:', users);
      const currentDate = new Date();
      const activeUsers = users.filter(user => {
        if (!user.endDate) return true; // 未设置 endDate 保留
        const endDate = new Date(user.endDate);
        return !isNaN(endDate.getTime()) && endDate >= currentDate;
      });
      console.log('过滤后的数据:', activeUsers);
      this.setData({
        activeUsers: this.data.activeUsers.concat(activeUsers),
        hasMoreUsers: res.data.length === pageSize
      });
      console.log('最终 activeUsers:', this.data.activeUsers);
    } catch (err) {
  ...
  async loadMoreUsers() {
    ...
    this.logOperation(`加载了更多${this.data.region}地区的正在帮教的孩子`);
  },
  ...
  async searchUsers() {
    ...
    try {
      ...
      const currentDate = new Date();
      const activeResults = searchResults.filter(user => !user.endDate || new Date(user.endDate) >= currentDate);
      this.setData({ searchResults: activeResults });
  ...
  goToUserDetails(e) {
    ...
    wx.navigateTo({ url: `/pages/userDetails/userDetails?openid=${openid}` });
  },
  ...
  async updateName() {
    ...
    try {
      ...
      this.setData({ showUpdateNameModal: false, selectedOpenid: '', newName: '', activeUsers: [], page: 0 });
      this.loadUsers();
      this.searchUsers();
  ...
  async updatePhone() {
    ...
    try {
      ...
      wx.showToast({ title: '更新成功', icon: 'success' });
      this.setData({ showUpdatePhoneModal: false, selectedOpenid: '', newPhone: '', activeUsers: [], page: 0 });
      this.loadUsers();
      this.searchUsers();
  ...
  async updateGuardianPhone() {
    ...
    try {
      ...
      wx.showToast({ title: '更新成功', icon: 'success' });
      this.setData({ showUpdateGuardianPhoneModal: false, selectedOpenid: '', newGuardianPhone: '', activeUsers: [], page: 0 });
      this.loadUsers();
      this.searchUsers();
  ...
  async updateEndDate() {
    ...
    try {
      ...
      wx.showToast({ title: '更新成功', icon: 'success' });
      this.setData({ showUpdateEndDateModal: false, selectedOpenid: '', newStartDate: '', newEndDate: '', activeUsers: [], page: 0 });
      this.loadUsers();
      this.searchUsers();
